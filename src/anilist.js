...
async function getAnilistTrending() {
    ...
    if (data && data.data && data.data.Page && data.data.Page.media) {
        data.data.Page.media = data.data.Page.media.map(item => {
            if (item.description) {
                item.description = item.description.replace(/<[^>]*>/g, '').replace(/\s\s+/g, ' ');
            }
            return item;
        });
    }
...
async function getAnilistUpcoming(page) {
    ...
    if (data && data.data && data.data.Page && data.data.Page.airingSchedules) {
        data.data.Page.airingSchedules = data.data.Page.airingSchedules.map(schedule => {
            if (schedule.media && schedule.media.description) {
                schedule.media.description = schedule.media.description
                .replace(/<[^>]*>/g, '')
                .replace(/\s\s+/g, ' ');
        }
            return schedule;
